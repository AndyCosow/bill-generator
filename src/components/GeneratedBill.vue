  ...
  <div id="bill" style="font-family: Arial,serif; padding: 20px;">
    ...
    <div style="margin-top: 50px">
      <table style="width:100%; border-collapse: collapse;">
        <tr>
          <th
            style="text-align: start; padding: 5px; border: 1px solid black"
            v-for="(item, index) in bill.table.header"
            :key="index"
          >
            {{ item[0] }}
          </th>
    ...
    <div style="display: flex; justify-content: space-between; margin-top: 100px" v-if="bill.showSignatures">
      <div style="width: 200px">
        ...
        <div>
          <small>{{ bill.props.sellerSign[0] }}</small>
        </div>
      </div>

      <div style="width: 200px">
        {{ bill.props.buyer[0] }}
        <div style="padding: 30px 0px" :style="signatureSpace(false)">{{ bill.signatures.buyer[0] }}</div>
        <div style="border-bottom: 1px solid black; margin-bottom: 5px"></div>
        <div>
          <small>{{ bill.props.buyerSign[0] }}</small>
        </div>
      </div>
    </div>
  </div>
